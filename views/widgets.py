...
from PySide6.QtGui import QPixmap, QPainter, QWheelEvent, QPen, QColor, QFont
...
class ImageViewer(QLabel):
    ...
    page_nav      = Signal(int)  # -1 = página anterior, +1 = siguiente

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setAlignment(Qt.AlignCenter)
        # Foco por clic/tab para poder navegar entre páginas con ← / →.
    ...
    def keyPressEvent(self, event):
        # ← / → navegan entre páginas cuando el visor tiene el foco. No afecta
        # a campos de texto/spin porque estos consumen las flechas antes.
...
class ThumbnailGrid(QListView):
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        self._recenter_columns()

    def _recenter_columns(self):
        # QListView's IconMode has no built-in way to center a partial row, so
        # inset the viewport symmetrically to absorb the leftover width instead.
        cell = CARD_W + self.spacing()
        vp_w = self.viewport().width() + self.viewportMargins().left() + self.viewportMargins().right()
        if vp_w <= 0 or cell <= 0:
            return
        cols = max(1, vp_w // cell)
        margin = max(0, (vp_w - cols * cell) // 2)
    ...
    def _ctx_menu(self, pos):
        ...
        menu.addSeparator()
        # Si hay 2+ páginas marcadas con su checkbox, la acción borra TODAS las
        # marcadas; si no, borra solo la fila del clic derecho.
        n_checked = len(self._checked)
        multi = n_checked >= 2
        act_del = menu.addAction(
            f"Eliminar {n_checked} páginas" if multi else "Eliminar página")
        ...
        if action == act_bm:
            ...
            if multi:
                # Borrar en orden descendente: al eliminar primero los índices
                # más altos, los más bajos siguen siendo válidos tras el
                # renumerado que hace cada eliminación (índice == fila).
                for i in sorted(self._checked, reverse=True):
                    self.page_deleted.emit(i)
                self._checked.clear()
