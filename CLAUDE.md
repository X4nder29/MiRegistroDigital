...
Building: use the `/build-exe` slash command (`.claude/commands/build-exe.md`) or `installer\build.bat` directly — it cleans `dist\`/`build\` and runs PyInstaller against `docscan_pro.spec`. Only `dist\MiRegistroDigital\MiRegistroDigital.exe` is a valid, runnable build; anything under `build\` is scratch space (see the command file for the exact pitfall).
...
- `digitization_page.py` (`DigitizationPage`) — the main scan/import/correct/OCR/export workflow. This is the largest and most central page.
